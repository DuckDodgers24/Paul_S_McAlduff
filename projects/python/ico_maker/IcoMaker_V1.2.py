...
class IcoMaker(QMainWindow):
    ...
    - Dropped files are filtered to only PNG images that are exactly 512×512
    ...
    def setup_ui(self):
        ...
        # Info icon (tooltip only), attached to the 512×512 line
        ...
        self.info_button.setIcon(QIcon())
    ...
    def resizeEvent(self, event):
        ...
        # Position the icon near the end of the "512×512 only" line.
    ...
    def apply_theme_icons(self):
        """
        Apply theme-aware window and info icons based on the current OS palette.

        This method selects the appropriate icon variants (white or black)
        depending on whether the application is running in dark mode or light mode.

        - In dark mode, light-colored (white) icons are used for visibility.
        - In light mode, dark-colored (black) icons are used.

        The icon files are expected to reside alongside the script.
        This method should be called after the UI is fully initialized,
        as theme detection relies on the finalized application palette.
        """
        if self.is_dark_mode():
            window_icon_name = "icomaker-icon-white.ico"
            info_icon_name = "info-circle-white.png"
        else:
            window_icon_name = "icomaker-icon-black.ico"
            info_icon_name = "info-circle-black.png"

        base = Path(__file__).resolve().parent

        self.setWindowIcon(QIcon(str(base / window_icon_name)))
        self.info_button.setIcon(QIcon(str(base / info_icon_name)))
